...
import { openQualityReport, downloadRulesAsYAML } from "@/lib/format-export";
        ...
        async function fetchAnalytics() {
            ...
            try {
                ...
                const numericCols = schema
                    ? schema.filter(c => c && c.column_type && /INT|FLOAT|DOUBLE|DECIMAL/i.test(c.column_type))
                        .map(c => c.column_name)
                    : [];

                if (numericCols.length >= 2) {
                    const validCols = numericCols.filter(col => col !== undefined && col !== "undefined").slice(0, 6);
                    if (validCols.length >= 2) {
                        const corr = await calculateCorrelationMatrix(activeTable, validCols);
                        setCorrelation(corr);
                    }
                }
            } catch (err) {
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-3">
                    <Button
                        variant="outline"
                        size="sm"
                        className="bg-white/[0.02] border-white/10 hover:bg-white/5 h-9"
                        onClick={() => {
                            if (activeTable && schema && qualityScore) {
                                openQualityReport(
                                    activeTable,
                                    schema as any,
                                    qualityScore.overallScore,
                                    qualityScore.columnScores
                                );
                            }
                        }}
                    >
                    ...
                    <Button
                        ...
                        onClick={() => {
                            if (activeTable) {
                                downloadRulesAsYAML(activeTable, [
                                    { column: "*", rule: "schema_check" }
                                ]);
                            }
                        }}
                    >
